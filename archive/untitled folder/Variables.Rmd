...
filtered_merged_df$ABSPID <- as.character(filtered_merged_df$ABSPID)
```
## Convert Specific Values (0, 97) to Missing Values (NA) in Factor Variables
```{r}
magic_vals <- c(99, 98, 97, 0)

# Iterate through columns to replace specific values with NA for factor variables
for (col in names(filtered_merged_df)) {
  if (is.factor(filtered_merged_df[[col]])) {
    ...
    # General case for replacing 99, 98, 97, 0 with NA
    filtered_merged_df[[col]][filtered_merged_df[[col]] %in% as.character(magic_vals)] <- NA
    
    # Special case for HBA1MREB and HBA1PREB where 7 and 8 should be treated as missing
    if (col %in% c("HBA1MREB", "HBA1PREB")) {
      filtered_merged_df[[col]][filtered_merged_df[[col]] %in% c("7", "8")] <- NA
    }
...
str(filtered_merged_df)
main <- filtered_merged_df
save(main, file = "ResponseVariables_Cleaned.RData")
```